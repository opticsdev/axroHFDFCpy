...
def VetTheCommand(CheckIt, command_line):
    """Checks if the command sent to the AXRO board is valid/legal.Returns
    "Illegal" if not valid

    Parameters
    ----------
    CheckIt : CommandCheck (obj)
        Class Instance containing acceptable commands and constraints on values  See CommandCheck.py
    command_line : str
        command sent to the AXRO Board
    ...
    if not command_line:
        ...
        CheckIt.AcceptableCommand(command_tokens)
...
#     print("\n    Where board_number ranges from ", acceptable_board_numbers[0],
#           " to ", acceptable_board_numbers[-1])
